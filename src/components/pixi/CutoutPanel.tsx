...
function CutoutPosition() {
    ...
    const onRetrieveCutout = () => {
        ...
        if (isDirty) {
            toaster.error({ title: "Unsaved changes", description: "Please confirm cutout position changes before retrieving cutout." });
            return;
        }
        refetchCutout();
    };
...
function ExtractionParameters() {
    ...
    return (
        <Stack gap={3}>
            <Heading size="sm" as='h3'>Extraction Parameters</Heading>
            <HStack gap={3}>
