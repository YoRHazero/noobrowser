...
function CollapseRegionIndicator({
	...
	const handleDrag = (
		delta: [number, number],
		mode: "move" | "top" | "bottom" | "left" | "right",
		isLast: boolean,
	) => {
		...
		const visibleWorldHeight =
			Math.abs(orthoCam.top - orthoCam.bottom) / orthoCam.zoom;
		const visibleWorldWidth =
			Math.abs(orthoCam.right - orthoCam.left) / orthoCam.zoom;
		...
		if (mode === "move" || mode === "top" || mode === "bottom") {
			if (mode === "move") {
				...
				if (newSpatialMax - newSpatialMin < minGap)
					newSpatialMin = newSpatialMax - minGap;
			} else if (mode === "bottom") {
				newSpatialMax += dataDeltaY;
				if (newSpatialMax > roiHeight) newSpatialMax = roiHeight;
				if (newSpatialMax - newSpatialMin < minGap)
					newSpatialMax = newSpatialMin + minGap;
			}
		}
		/* --------------------------------- X axis --------------------------------- */
	...
	const [hoveredPart, setHoveredPart] = useState<
		null | "move" | "ns-edge" | "ew-edge"
	>(null);

	useEffect(() => {
		if (isDragging) return;
		if (hoveredPart === "move") document.body.style.cursor = "grab";
		else if (hoveredPart === "ns-edge")
			document.body.style.cursor = "ns-resize";
		else if (hoveredPart === "ew-edge")
			document.body.style.cursor = "ew-resize";
		else document.body.style.cursor = "auto";
		return () => {
			document.body.style.cursor = "auto";
		};
	}, [hoveredPart, isDragging]);
