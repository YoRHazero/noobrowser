...
export default function LinearModelCard(props: LinearModelCardProps) {
	...
	const { steps, setSingleStep } = useFitStepScaler({
		initialSteps: { k: 0.1, b: 0.1, x0: waveUnit === "µm" ? 0.001 : 1, range: waveUnit === "µm" ? 0.001 : 1 },
		waveUnit,
		waveFrame,
		zRedshift,
	});
	...
	const handleX1 = createHandler((v) =>
		updateModel(model.id, { range: { ...model.range, min: v } as any }),
	);
	const handleX2 = createHandler((v) =>
		updateModel(model.id, { range: { ...model.range, max: v } as any }),
	);
	...
	// 5. Display Values
	const displayX0 = toDisplayWavelength(model.x0, waveUnit, waveFrame, zRedshift);
	...
	return (
		<ModelCardShell
			...
			stepControls={[
				{ key: "k", label: "k step", value: steps["k"], onChange: (v) => setSingleStep("k", v) },
				{ key: "b", label: "b step", value: steps["b"], onChange: (v) => setSingleStep("b", v) },
				{ key: "x0", label: "x0 step", value: steps["x0"], onChange: (v) => setSingleStep("x0", v) },
		...
		>
			<Stack gap={1}>
				<Stack direction="row" gap={2}>
					<CompactNumberInput label="k" value={model.k} step={steps["k"]} onChange={handleK} decimalScale={6} />
					<CompactNumberInput label="b" value={model.b} step={steps["b"]} onChange={handleB} decimalScale={6} />
				</Stack>
				<CompactNumberInput label="x0" value={displayX0} step={steps["x0"]} onChange={handleX0} />
				<Stack direction="row" gap={2}>
					<CompactNumberInput
						label="x1"
						value={displayX1}
