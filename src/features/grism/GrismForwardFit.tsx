...
export default function GrismForwardFit() {
    return (
        <Box display="inline-flex" flexDirection="column" gap={4} p={4} border="1px solid black" borderRadius="md">
...
function LinearModelCard(props: LinearModelCardProps) {
    ...
    useEffect(() => {
        ...
        if (!Number.isFinite(oldFactor) || oldFactor === 0 || !Number.isFinite(newFactor)) {
...
function GaussianModelCard(props: GaussianModelCardProps) {
    ...
    const fwhmInUnit =
        waveUnit === "µm" ? fwhmUm : fwhmUm * ANGSTROM_PER_MICRON;
    const fwhmVelocity =
