...
import { Switch, Box, HStack, Separator } from "@chakra-ui/react";
...
function GrismNormSlider() {
    ...
    const handlePminChange = (newPmin: number) => {
        ...
        if (extractedSpecSortedArray) {
            const vmin = percentileFromSortedArray(extractedSpecSortedArray, pmin) as number;
    ...
    const handlePmaxChange = (newPmax: number) => {
        ...
        if (extractedSpecSortedArray) {
            const vmax = percentileFromSortedArray(extractedSpecSortedArray, pmax) as number;
...
function GrismNormSwitch() {
    ...
    return (
        <Tooltip 
            ids={{ trigger: id }}
            content={normInWindow ? "Switch to use entire range" : "Switch to use current window range"}
        >
            <Switch.Root
                ids={{ root: id }}
