...
class FootprintContainer extends Container {
	...
	public renderFrame(
		...
		this.footprints.forEach((fp) => {
			...
			if (isSelected) {
				graphics.stroke({ color: COLORS.strokeSelected, width: 2 });
				graphics.zIndex = 3;
			} else if (isHovered) {
				graphics
				...
				graphics.stroke({ color: COLORS.strokeNormal, width: 1 });
				graphics.zIndex = 1;
			}
		});
	}
...
export default function FootprintManager({
	...
	useEffect(() => {
		if (!footprintManagerRef.current) return;
		footprintManagerRef.current.updateInteractiveState(
			hoveredFootprintId,
			selectedFootprintId,
		);
	...
	useTick(() => {
		if (footprintManagerRef.current && app) {
			footprintManagerRef.current.renderFrame(viewRef.current, globeBackground);
		}
	});

	return <pixiFootprintContainer ref={footprintManagerRef} />;
}
