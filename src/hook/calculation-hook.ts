...
export function useRoiSpectrum1D(basename: string | undefined) {
	const { roiCollapseWindow, roiState } = useGrismStore(
		useShallow((state) => ({
	...
	const spectrum1D = useMemo<Spectrum1D[]>(() => {
		...
		const waveRange = [
			clamp(roiCollapseWindow.waveMin, 0, roiWidth),
		...
		const spatialRange = [
			clamp(roiCollapseWindow.spatialMin, 0, roiHeight),
			clamp(roiCollapseWindow.spatialMax, 0, roiHeight),
		];
		const spatialMin =
			spatialRange[0] < spatialRange[1] ? spatialRange[0] : spatialRange[1];
		const spatialMax =
			spatialRange[0] < spatialRange[1] ? spatialRange[1] : spatialRange[0];
		...
		for (let col = startCol; col < endCol; col++) {
			...
			for (let row = startRow; row < endRow; row++) {
				...
				if (Number.isNaN(flux) || Number.isNaN(err)) continue;
				fluxSum += flux;
				errSumSq += err * err;
				count += 1;
			}
